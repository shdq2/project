function w_resize() {
	var p_w = $('.user-guest').css('width');
	var l_w = $('.guest-img').css('width');
	var ph_w = $('.photo').css('width');
	$('.user-guest').css('height',p_w);
	$('.user-guest-list img').css('height',l_w);
	$('.photo').css('height',ph_w+50);
	if(width <= 768) {
		$('.guest-container').css('padding', '0px');
		$('.guest-nav').css('display', 'none');
		$('.guest-main').css('width', '100%');
		$('.guest-main').css('left', '0%');
		$('.guest-main').css('border-left', '0px');
		$('.col-main').removeClass('col-xs-9').addClass('col-xs-12');
		$('.col-img').removeClass('col-xs-3').addClass('col-xs-12');
		$('.guest-container').css('transition', 'transform 0.3s ease');
		
	} else {
		$('.guest-container').css('padding', '0px 0px 0px 15px');
		$('.guest-nav').css('display', 'inline-block');
		$('.guest-main').css('width', 'calc(100% - 220px)');
		$('.guest-main').css('left', '220px');
		$('.guest-main').css('border-left', '1px solid #f2f2f2');
		$('.col-main').removeClass('col-xs-12').addClass('col-xs-9');
		$('.col-img').removeClass('col-xs-12').addClass('col-xs-3');
		$('.guest-container').css('transition', 'transform 0s ease');
		
		$('.tmenu').css('transform', 'translateX(-270px)');
		$('.guest-container').css('transform', 'translateX(0px)');
		
		$('.user-guest-list').css('height','');
	}
	if(width>=992){
...
$(function() {
	...
	$('.topbar-menu').click(function() {
		...
		if(box == 'topbar_menu.png")') {
			...
			$('.guest-container').css('transform', 'translateX(270px)');
		}
